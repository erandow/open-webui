<script>
	...
	const saveHandler = async (data) => {
		...
		if (compareVersion(manifest?.required_open_webui_version ?? '0.0.0', WEBUI_VERSION)) {
			...
			toast.error(
				$i18n.t(
					'Majid version (v{{OPEN_WEBUI_VERSION}}) is lower than required version (v{{REQUIRED_VERSION}})',
					{
		...
		const res = await updateFunctionById(sessionStorage.token, func.id, {
			id: data.id,
			name: data.name,
			meta: data.meta,
	...
	onMount(async () => {
		...
		if (id) {
			func = await getFunctionById(sessionStorage.token, id).catch((error) => {
				toast.error(`${error}`);
