<script lang="ts">
	...
	const getUserValves = async () => {
		loading = true;
		if (tab === 'tools') {
			valves = await getToolUserValvesById(sessionStorage.token, selectedId);
			valvesSpec = await getToolUserValvesSpecById(sessionStorage.token, selectedId);
		} else if (tab === 'functions') {
			valves = await getFunctionUserValvesById(sessionStorage.token, selectedId);
			valvesSpec = await getFunctionUserValvesSpecById(sessionStorage.token, selectedId);
		}
	...
	const submitHandler = async () => {
		if (valvesSpec) {
			...
			if (tab === 'tools') {
				const res = await updateToolUserValvesById(sessionStorage.token, selectedId, valves).catch(
					(error) => {
						toast.error(`${error}`);
						return null;
					}
